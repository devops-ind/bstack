...
def checkout_existing_branch(repo_path, branch_name):
    """
    Checkout an existing branch and pull latest changes

    This is used when committing directly to main/target branch.
    ...
    try:
        ...
        # Pull latest changes
        run_command(['git', 'pull', 'origin', branch_name], cwd=repo_path)

        print(f"✅ Branch checked out: {branch_name}")

    except subprocess.CalledProcessError as e:
        print(f"❌ Branch checkout failed: {e}")
        raise
...
def run_upload_workflow(params, config_file='config/config.yaml'):
    ...
    try:
        ...
        print("🌿 STEP 5: Setup Git Branch")
        print("-" * 70)

        # Check if we should create a PR or commit directly
        create_pr = config['git'].get('create_pr', True)
        ...
        if create_pr:
            ...
            print(f"📝 Will create Pull Request from: {branch_name}")
        else:
            # Commit directly to target branch
            branch_name = config['git'].get('target_branch', 'main')
            checkout_existing_branch(repo_path, branch_name)
            print(f"📝 Will commit directly to: {branch_name}")
        ...
        # STEP 8: Create Pull Request (if enabled)
        pr_url = None
        if create_pr:
...
"""
            ...
            print("🔀 STEP 8: Create Pull Request")
            print("-" * 70)
            print("⏭️  Skipped - create_pr is disabled in config")
            print(f"✅ Changes committed directly to: {branch_name}")
            print()
        ...
        # For Teams notification, use a placeholder if no PR was created
        teams_pr_url = pr_url if pr_url else params['source_build_url']
        ...
        if pr_url:
            ...
            print(f"✅ Committed directly to: {branch_name}")
        print("="*70 + "\n")
